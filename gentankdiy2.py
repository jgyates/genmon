...
class GenTankData(MySupport):
    ...
    def __init__(self,
        ...
        try:
            if not os.path.isfile(configfile):
                err_msg = "Missing config file : {0}".format(configfile)
            ...
            if len(init_err_msg):
                self.LogError("InitADC failed, '{0}'.format(init_err_msg), exiting")
                sys.exit(1)
            ...
            err_msg = "Error reading {0}: {1}".format(configfile, str(e1))
        ...
        try:
            ...
            err_msg = "Error in GenTankData init: {0}".format(str(e1))
            self.LogErrorLine(err_msg)
    ...
    def SendCommand(self, Command):
        ...
        try:
            ...
            self.LogErrorLine("Error calling  ProcessMonitorCommand: {0}".format(str(Command)))
            data = ""
    ...
    def TankCheckThread(self):
        ...
        while True:
            try:
                ...
                self.LogErrorLine(("Error in '{self.TankCheckThread.__name__}': {1)}").format(self.TankCheckThread.__name__, str(e1)))
...
def SetupStandAloneProgram(prog_name):
    ...
    try:
        ...
        log = SetupLogger("client", os.path.join(loglocation, prog_name + ".log"))
    except Exception as e1:
...
if __name__ == "__main__":
    ...
    #MySupport.CheckRootPrivileges(console)
    ...
    #port, loglocation = MySupport.GetGenmonInitInfo(ConfigFilePath, log = console)
    #log = SetupLogger("client", os.path.join(loglocation, "gentankdatadiy2.log"))

    console, ConfigFilePath, address, port, loglocation, log = SetupStandAloneProgram("gentankdiy2")
