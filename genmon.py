...
class Monitor(MySupport):
    ...
    def ProcessCommand(self, command, fromsocket=False):
        ...
        try:
            ...
            CommandDict = {
                ...
                "sensor_log_json": [
                    self.Controller.GetSensorHistory,
                    (command.lower(),),
                    True,
                ],
                "sensor_names_json": [
                    self.Controller.GetSensorNames,
                    (),
                    True,
                ],
                "sensor_log_clear": [self.Controller.ClearSensorLog, (), True],
    ...
    def DisplayHelp(self):
        ...
        outstring += self.printToString(
            "                  if Enhanced Exercise Frequency is supported by"
        )
        outstring += self.printToString("                  your generator:")
        outstring += self.printToString(
            "                      i.e. setexercise=Monday,13:30,BiWeekly"
        )
        outstring += self.printToString(
            "                      i.e. setexercise=15,13:30,Monthly"
        )
        outstring += self.printToString(
        ...
        outstring += self.printToString(
            "   sendlogfiles - Send log files to the developer if outbound email is setup."
        )
        ...
        outstring += self.printToString(
            "To clear the Alarm/Warning message, press OFF on the control panel keypad"
        )
        outstring += self.printToString(
            "followed by the ENTER key. To access Dealer Menu on the Evolution"
        )
        outstring += self.printToString(
            "controller, from the top menu selection (SYSTEM, DATE/TIME,BATTERY, SUB-MENUS)"
        )
        outstring += self.printToString(
            "enter UP UP ESC DOWN UP ESC UP, then go to the dealer menu and press enter."
        )
        outstring += self.printToString(
            "For liquid cooled models a level 2 dealer code can be entered, ESC UP UP DOWN"
        )
        outstring += self.printToString(
            "DOWN ESC ESC, then navigate to the dealer menu and press enter."
        )
        outstring += self.printToString(
            "Passcode for Nexus controller is ESC, UP, UP ESC, DOWN, UP, ESC, UP, UP, ENTER."
        )
        outstring += self.printToString("\n")
    ...
    def GetProgramRunTime(self):
        try:
            ...
            outstr = str(ProgramRunTime).split(".")[
                0
    ...
    def DisplayMonitor(self, DictOut=False, JSONNum=False):
        ...
        try:
            ...
            GenMonStats.append(
                {"Controller": self.Controller.GetController(Actual=False)}
            )
            ...
            GenMonStats.append(
                {"Generator Monitor Version": ProgramDefaults.GENMON_VERSION}
            )
            GenMonStats.append(
            ...
            if self.bSyncDST:
                if self.bDST != self.is_dst():  # has DST changed?
                    ...
                    self.MessagePipe.SendMessage(
                        ...
                        "Time updated due to daylight savings time change",
                        msgtype="info",
                    )
    ...
    def ComWatchDog(self):
        ...
        while True:
            ...
            if counter > 30:
                self.LogError(
                    "WARNING: Initialization not complete after 30 seconds, possible communication failure. Check cabling."
                )
                break
            counter += 1
            if self.Controller.InitComplete:
            ...
            try:
                ...
                if self.CommunicationsActive:
                    ...
                    if NoticeSent:
                        NoticeSent = False
                        msgbody = (
                            "Generator Monitor communications with the controller has been restored at "
                            + self.SiteName
                        )
                        msgbody += "\n" + self.DisplayMonitor()
                        self.MessagePipe.SendMessage(
                    ...
                    ) > (1 + self.AdditionalWatchdogTime):
                        if not NoticeSent:
                            NoticeSent = True
                            msgbody = (
                                "Generator Monitor is not communicating with the controller at "
                                + self.SiteName
                            )
                            msgbody += "\n" + self.DisplayMonitor()
                            self.MessagePipe.SendMessage(
        ...
        try:
            if (
                self.GetDeltaTimeMinutes(
                    datetime.datetime.now() - self.LastSoftwareUpdateCheck
                )
                > 1440
            ):  # check every day
                self.LastSoftwareUpdateCheck = datetime.datetime.now()
                            ...
                            for value in quoted.findall(line):
                                if value != ProgramDefaults.GENMON_VERSION:
                                    ...
                                    msgbody = (
                                        ...
                                        + ". The new version ("
                                        + value
                                        + ") can be updated on the About page of the web interface. The current version installed is "
                                        + ProgramDefaults.GENMON_VERSION
                                        + ". You can disable this email from being sent on the Settings page."
                                    )
                                    if len(self.UserURL):
                                        msgbody += (
        ...
        while True:
            try:
                ...
                SocketThread = threading.Thread(
                    target=self.SocketWorkThread, args=(conn,), name="SocketWorkThread"
                )
        ...
        try:
            ...
            for name, object in self.Threads.items():
                try:
                    ...
                    self.LogErrorLine(
                        "Error killing thread in Monitor Close: " + name + ":" + str(e1)
                    )
