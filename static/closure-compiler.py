...
def compress(compiler, in_files, out_file, in_type="js"):
    ...
    if in_type == "js":
        print(
            ...
            % (compiler, '" --js "'.join(in_files), out_file)
        )
        os.system(
            'java -jar %s --js "%s" --js_output_file "%s"'
            % (compiler, '" --js "'.join(in_files), out_file)
    ...
    if in_type == "css":
        print(
            'java -jar %s --allow-unrecognized-functions --allow-unrecognized-properties  --output-file "%s" "%s"'
            % (compiler, out_file, '" "'.join(in_files))
        )
        os.system(
            'java -jar %s --allow-unrecognized-functions --allow-unrecognized-properties --output-file "%s" "%s"'
            % (compiler, out_file, '" "'.join(in_files))
...
def main():
    ...
    r = requests.get(
        "https://repo1.maven.org/maven2/com/google/javascript/closure-compiler/v20200719/closure-compiler-v20200719.jar",
