...
class GaugeDIY(MySupport):
    ...
    def __init__(self, config, log=None, console=None):
        ...
        self.i2c_channel = self.config.ReadValue(
            "i2c_channel", return_type=int, default=1
        )
        available_i2c_channels = [
            int(n[len(self.I2C_DEV_PREFIX) :])
            for n in glob.glob(self.I2C_DEV_PREFIX + "[0-9]*")
        ]
        assert (
            available_i2c_channels
        ), "Is i2c enabled? No available i2c channels found."
        assert self.i2c_channel is not None, "Invalid i2c channel."
        assert (
            self.i2c_channel in available_i2c_channels
        ), "i2c channel error: 'i2c_channel' must be one of {}".format(
            available_i2c_channels
        )

        self.i2c_address = self.config.ReadValue(
            "i2c_address", return_type=int, default=72
        )  # 0x48
...
class GaugeDIY1(GaugeDIY):
    ...
    def __init__(self, config, log=None, console=None):
        ...
        self.mv_per_step = self.config.ReadValue(
            "mv_per_step", return_type=float, default=187.5
        )
        self.Multiplier = self.config.ReadValue(
            "volts_to_percent_multiplier", return_type=float, default=20.0
        )
        self.debug = self.config.ReadValue("debug", return_type=bool, default=False)
        self.simulate = self.config.ReadValue(
            "simulate", return_type=bool, default=False
        )
    ...
    def GetGaugeData(self, tanknum=0):
        try:
            ...
            time.sleep(
                1
            )  # Wait a second so the Analog-to-Digital Converter has time to process the command.

            val = self.I2Cbus.read_i2c_block_data(
                self.i2c_address, self.POINTER_CONVERSION, 2
            )
...
class GaugeDIY2(GaugeDIY):
    ...
    def __init__(self, config, log=None, console=None):
        ...
        self.adc_COS_inp = self.config.ReadValue(
            "adc_COS_inp", return_type=int, default=0
        )
        assert self.adc_COS_inp in [0, 1, 2, 3], "'adc_COS_inp' must be 0,1,2, or 3"

        self.adc_SIN_inp = self.config.ReadValue(
            "adc_SIN_inp", return_type=int, default=1
        )
        assert self.adc_SIN_inp in [0, 1, 2, 3], "'adc_SIN_inp' must be 0,1,2, or 3"

        self.adc_VDD_inp = self.config.ReadValue(
            "adc_VDD_inp", return_type=int, default=None
        )
        assert self.adc_VDD_inp in [
            0,
            1,
            2,
            3,
            None,
        ], "'adc_VDD_inp' must be 0,1,2, or 3"
        ...
        for i in range(1, self.MAX_AP_TABLE + 1):
            ...
            if ap != None:
                ...
                try:
                    ...
                    assert (
                        p >= 0.0 and p <= 100.0
                    ), "{0}{1} percent ({2}) must be between 0 and 100".format(
                        self.ANG_PCT_PNT_, i, p
                    )
                    self.ap_table.append((float(apl[0]), p))
                except Exception as e1:
                    self.LogConsole(
                        "Invalid value in config file '{0}', line for: {1}{2} {3}".format(
                            self.configFileName, self.ANG_PCT_PNT_, i, ap
                        )
                    )
                    self.LogErrorLine("Error in GaugeDIY2:Init " + str(e1))

        if len(self.ap_table) > 1:
            self.ap_table.sort(key=lambda x: x[1])
    ...
    def InitADC(self):
        ...
        try:
            ...
            try:
                ...
                self.LogErrorLine(
                    "Unable to open SMBus device for I2C channel "
                    + str(self.i2c_channel)
                )
                return False
        ...
        if len(self.ap_table) < 2:
            err_msg += "{0}x table needs at least 2 entries, only {1} found".format(
                self.ANG_PCT_PNT_, len(self.ap_table)
            )
    ...
    def _read_voltage(self, pin):
        ...
        self.I2Cbus.write_word_data(
            self.i2c_address,
            self.POINTER_CONFIGURATION,
            self.CONFIG_VALUE | ((pin + 0x04) << 4),
        )

        # Wait for conversion to complete
        while (
            not self.I2Cbus.read_word_data(self.i2c_address, self.POINTER_CONFIGURATION)
            & 0x8000
        ):
            pass
    ...
    def convert_angle_to_percent(self, ain):
        ...
        if self.ap_table == None or len(self.ap_table) < 2:
            return -1.0
        ...
        ts = -0.01 if apt[0][1] > apt[-1][1] else 0.1
