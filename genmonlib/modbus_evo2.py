...
class ModbusEvo2(ModbusProtocol):
    ...
    def __init__(
        ...
        super(ModbusEvo2, self).__init__(
            updatecallback=self.ParentCallback,
            address=address,
            name=name,
            rate=rate,
            Parity=None,
        ...
        try:
            ...
            if self.config != None:
                self.UnlockInterval = self.config.ReadValue(
                    "evo2_unlock_interval", return_type=int, default=3
                )
            ...
            self.iv = (
                b"\xC0\x94\xFB\xEB\xF5\x96\x43\x7F\xA2\x2E\xFA\x84\xFC\xC5\x21\x52"
            )

            # 16 keys
            self.key = [
                b"\x4A\x2A\xA3\xE4\x7E\xE0\x42\x2C\xA4\xBC\x8D\x1D\x52\xDE\xD9\x69",
    ...
    def ProcessTransaction(
        ...
        if self.Encapsulating():
            self.SendUnlockSequence(
                Register, Length, skipupdate=False, ReturnString=False
            )
    ...
    def SendUnlockSequence(
        self, Register, Length, skipupdate=False, ReturnString=False
    ):

        try:
            if not (
                self.ExcepAck != self.LastExectpionCount
                or self.LastUnlockTime == None
                or self.GetDeltaTimeMinutes(
                    datetime.datetime.now() - self.LastUnlockTime
                )
                > self.UnlockInterval
            ):
                return False
            ...
            try:
                ...
                self._PWT(
                    self.ModbusEncapsulationRegister,
                    len(Data) / 2,
                    Data,
    ...
    def GetControlBytes(self, Packet):
        ...
        if Packet[self.MBUS_OFF_COMMAND] == self.MBUS_CMD_READ_REGS:
            ...
            return [
                Packet[self.MBUS_OFF_WRITE_REG_REQ_DATA],
                Packet[self.MBUS_OFF_WRITE_REG_REQ_DATA + 1],
            ]
        else:
    ...
    def NybbleSwap(self, Buffer):
        ...
        try:
            ...
            if len(Buffer) != (blocksize * 2):
                self.LogError(
                    "Error in NybbleSwap: Invalid buffer length : " + str(len(Buffer))
                )
