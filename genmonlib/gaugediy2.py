...
class GaugeDIY2():
    ...
    def __init__(self, config):
        ...
        for i in range(1, self.MAX_AP_TABLE+1):
            ap = config.ReadValue("{0}{1}".format(self.ANG_PCT_PNT_, i))
            if ap != None:
                apl = ap.split(",")
                try:
                    p = float(apl[1])
                    ...
                    print("Invalid value in config file '{0}', line for: {1}{2} {3}".format(config.FileName, self.ANG_PCT_PNT_, i, ap))
...
# ---------- GaugeDIY2::GetAvgGaugeAngle--------------------------------------
    def GetAvgGaugeAngle(self, average_n = 10, secs = 1.0, show_status = False):
        ...
        if show_status: 
            print("Reading sensor", end ="")
            sys.stdout.flush()

        a = 0.0
        for i in range(average_n):
            a += self.read_gauge_angle()
            if show_status: print(".",end = "")
            sys.stdout.flush()
