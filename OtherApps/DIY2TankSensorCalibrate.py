    ...
    if gauge.ap_table != None and len(gauge.ap_table) > 2:
        ...
        for i in range(1, len(gauge.ap_table)):
            if (increasing and gauge.ap_table[i - 1][0] < gauge.ap_table[i][0]) or (
                ...
                print(
                    (
                        "error in calibration table at row {0}, {1:.2f}"
                        + " is {2} than {3:.2f}\n"
                        + "value not added to calibration table."
                    ).format(
                        i,
                        gauge.ap_table[i - 1][0],
                        "greater" if increasing else "less",
                        gauge.ap_table[i][0],
                    )
                )
...
"""
        ...
        if c == "1":
            ...
            p = input_float(
                "Set dial as low as possible and enter the percent value (typically 5%)\n"
            ...
            if p != None:
                ...
                p = input_float(
                    "Set dial as high as possible and enter the percent value (typically 100%)\n"
                    + "when enter is pressed after the value is entered the dial angle will be read and recorded: "
                )
                if p != None:
                    if p < gauge.ap_table[0][1]:
                        print(
                            "Error: the low entry must be less than the high entry, {0:.1f} is not less than {1:.1f}".format(
                                gauge.ap_table[0][1]
                            ),
                            p,
                        )
                        ...
                        show_ap_table()
                        print(
                            "Next use option 4 to add calibration points between {0:.1f} and {1:.1f}".format(
                                gauge.ap_table[0][1], gauge.ap_table[1][1]
                            )
                        )
                        print(
                            "suggested to use major markings on dial, ie 10, 20, 30, 40, 50, 60, 70"
                        )
            ...
            if gauge.ap_table == None or len(gauge.ap_table) < 2:
                ...
                if len(gauge.ap_table) >= gauge.MAX_AP_TABLE:
                    print(
                        "already maximum of {0} entries, delete some (5) or all (2)".format(
                            gauge.MAX_AP_TABLE
                        )
                    )
                else:
                    while len(gauge.ap_table) < gauge.MAX_AP_TABLE:
                        while True:
                            p = input_float(
                                "Set dial to desired value to read the associated angle for\n"
            ...
            if gauge.ap_table != None and len(gauge.ap_table) > 0:
                while True:
                    ...
                    if i >= 1 and i <= len(gauge.ap_table):
                        ...
                        print(
                            "Index must be from 1 to {0}, try again".format(
                                len(gauge.ap_table)
                            )
                        )
            ...
            if gauge.ap_table != None:
                if len(gauge.ap_table) < 2:
                    ...
                    for a, p in gauge.ap_table:
                        config.WriteValue(
                            "ang_pct_pnt_{0}".format(i),
                            "{0:8.2f},{1:6.1f}".format(a, p),
                        )
                        i += 1

                    print(
                        "wrote {0} calibration entries in '{1}'".format(
            ...
            print(
                "Current dial angle is {0:.2f} degrees which is {1:.1f}%".format(
                    a, gauge.convert_angle_to_percent(a)
                )
            )
